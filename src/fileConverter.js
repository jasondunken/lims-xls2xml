...
function translateXLS(window, data) {
  ...
  const start =
    `<DataExchangeTemplate RootArtifact="${rootArtifact}" Description="" IgnoreResultStateDuringImport="False" HideFromExportMenu="False">\n` +
    `\t<Artifact Name="${rootArtifact}" Type="${rootArtifact}" ImportType="${data.importType}" Required="yes">\n`;
  ...
  for (let name of names) {
    ...
    for (let i = name.length - 1; i >= 2; i--) {
      ...
      const propertyType =
        i == name.length - 1 ? "SimpleProperty" : "ComplexProperty";
